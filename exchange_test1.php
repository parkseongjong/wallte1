...
$eth->getBalance($walletAddress, function ($err, $balance) use (&$getNewBalance,&$langArr) {
		...
		if ($err !== null) {
			...
			header('location: exchange_test1.php');
			exit();
		}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	if($_SESSION['user_id']==45){
		...
		], function ($err, $transaction) use ($eth, $fromAccount, $toAccount, &$actualAmountToSendWithoutDecimal,&$ethTransactionId,&$langArr) {
			if ($err !== null) {
				//$_SESSION['failure'] = "Unable to Transfer Eth to Admin Account.";
				$_SESSION['failure'] = !empty($langArr['exchange_message3']) ? $langArr['exchange_message3'] : 'Unable to Transfer Eth to Admin Account.'; // (2020-05-22, YMJ)
				header('location: exchange_test1.php');
				exit();
			}
