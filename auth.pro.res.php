...
/* ============================================================================== */
/* =   인증데이터 수신 및 복호화 페이지 : 회원정보수정(profile)          = */
/* = -------------------------------------------------------------------------- = */
	...
	if( $cert_enc_use == "Y" ) {
		if( $res_cd == "0000" ) {
			...
			try {
				...
				//if ( !empty($ct_cert->mf_get_key_value("phone_no") ) && !empty($param_opt_3) ) {
				if ( !empty($param_opt_3) ) {
					...
					// param_opt_2(admin_type), param_opt_3(user_id) 
					...
					$db = getDbInstance();
					$db->where("auth_phone", $auth_phone_no);
					//$db->where("id_auth", 'Y');
					...
					if ($db->count == 0) { 
						...
						$_val_u['auth_phone'] = $auth_phone_no;
						$_val_u['auth_name'] = $auth_name;

						if ($auth_gender) {
							$_val_u['auth_gender'] = $auth_gender;
						}
						if ($auth_dob_y && $auth_dob_m && $auth_dob_d) {
							$_val_u['auth_dob'] = $auth_dob_y.'-'.$auth_dob_m.'-'.$auth_dob_d;
						}
						if ($auth_local_code) {
							$_val_u['auth_local_code'] = $auth_local_code;
						}
						...
						if ($auth_phone_no != '' ) {
							$last_id = $db2->update('admin_accounts', $_val_u);
							$_SESSION['success'] = $langArr['auth_success'];
							$_SESSION['auth_re'] = 'fin';
						} else {
							// 인증실패
