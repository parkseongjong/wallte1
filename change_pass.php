...
if ($_SERVER['REQUEST_METHOD'] == 'POST') 
{
	...
	if(trim($_POST['new_pass'])!="" && ($_POST['new_pass'] == $_POST['conf_pass'])){
	   ...
	   // 이전 패스워드가 일치하지 않아도 비밀번호가 변경되는 문제 발생하여 수정함(Fixed a problem where the password is changed even if the previous password does not match) (2020.05.13, YMJ)
		$db = getDbInstance();
		$db->where("id", $_SESSION['user_id']);
		$db->where("passwd", md5($_POST['old_pass']));
					...
					<fieldset>
						...
						<div class="form-group">
							<label for="f_name"><?php echo !empty($langArr['new_password']) ? $langArr['new_password'] : "New Password"; ?> *</label>
							  <input type="password" name="new_pass" value="" pattern=".{6,}" title="Must contain at least 6 or more characters"  class="form-control" required="required" id = "new_pass" >
						</div> 
						<div id="message">
