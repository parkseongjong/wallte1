...
if ($_SERVER['REQUEST_METHOD'] === 'POST') 
{
    ...
    if ($db->count >= 1) {
		 ...
		 if($userId == 1393){
			...
			if($db->count==0 && $userRole!='admin' && $registerWith!="email"){
				...
				$eth->sendTransaction([
					...
					//'value' => '0xCCA2E51310000',
					'value' => '0xFFCB9E57D4000',
					'gas' => '0x186A0',   //100000
			...
			$db = getDbInstance();
			$db->where ("user_id", $userId);
			$db->where ("coin_type", 'tp');
			$db->where ("ethmethod", 'approve');
			$ethSendRow = $db->get('ethsend');
			
			if($db->count==0 && $userRole!='admin' && $registerWith!="email"){
				
				$contractAddress = $contractAddressArr['tp3']['contractAddress'];
				$testAbi = $contractAddressArr['tp3']['abi'];
				$approveTxId = '';
				$contract = new Contract($web3->provider, $testAbi);
				$senderAccount = $adminAccountWalletAddress;
				$ownerAccount = $walletAddress;
				...
				if(!empty($approveTxId)) {		
					...
					$db = getDbInstance();
					$db->where("id", $userId);
					$last_id = $db->update('admin_accounts', ['tp_approved'=>"Y"]);
				}
			}
			...
			$db = getDbInstance();
			$db->where ("user_id", $userId);
			$db->where ("coin_type", 'usdt');
			$db->where ("ethmethod", 'approve');
			$ethSendRow = $db->get('ethsend');
			
			if($db->count==0 && $userRole!='admin' && $registerWith!="email"){
				
				$contractAddress = $contractAddressArr['usdt']['contractAddress'];
				$testAbi = $contractAddressArr['usdt']['abi'];
				$approveTxId = '';
				$contract = new Contract($web3->provider, $testAbi);
				$senderAccount = $adminAccountWalletAddress;
				$ownerAccount = $walletAddress;
				...
				if(!empty($approveTxId)) {		
					...
					$db = getDbInstance();
					$db->where("id", $userId);
					$last_id = $db->update('admin_accounts', ['usdt_approved'=>"Y"]);
				}
			}
			...
			$db = getDbInstance();
			$db->where ("user_id", $userId);
			$db->where ("coin_type", 'krw');
			$db->where ("ethmethod", 'approve');
			$ethSendRow = $db->get('ethsend');
			
			if($db->count==0 && $userRole!='admin' && $registerWith!="email"){
				
				$contractAddress = $contractAddressArr['krw']['contractAddress'];
				$testAbi = $contractAddressArr['krw']['abi'];
				$approveTxId = '';
				$contract = new Contract($web3->provider, $testAbi);
				$senderAccount = $adminAccountWalletAddress;
				$ownerAccount = $walletAddress;
				...
				if(!empty($approveTxId)) {		
					...
					$db = getDbInstance();
					$db->where("id", $userId);
					$last_id = $db->update('admin_accounts', ['krw_approved'=>"Y"]);
				}
			}
			...
			$db = getDbInstance();
			$db->where ("user_id", $userId);
			$db->where ("coin_type", 'mc');
			$db->where ("ethmethod", 'approve');
			$ethSendRow = $db->get('ethsend');
			
			if($db->count==0 && $userRole!='admin' && $registerWith!="email"){
				
				$contractAddress = $contractAddressArr['mc']['contractAddress'];
				$testAbi = $contractAddressArr['mc']['abi'];
				$approveTxId = '';
				$contract = new Contract($web3->provider, $testAbi);
				$senderAccount = $adminAccountWalletAddress;
				$ownerAccount = $walletAddress;
				...
				if(!empty($approveTxId)) {		
					...
					$db = getDbInstance();
					$db->where("id", $userId);
					$last_id = $db->update('admin_accounts', ['mc_approved'=>"Y"]);
				}
			}
		...
		else if($sendApproved=='N' && $userRole!='admin' && $registerWith=="email") {
